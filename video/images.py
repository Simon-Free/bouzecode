# [desc] Image generation backends for video pipeline: Gemini Web, web search, and placeholder fallback. [/desc]
"""
...
def _find_gemini_image_gen() -> str | None:
    ...
    # The bouzecode package lives at: .../bouzecode_versions/bouzecode/video/
    # v-content-creator lives at:         .../bouzecode_versions/v-content-creator/
    this_dir     = os.path.dirname(os.path.abspath(__file__))
    pkg_root     = os.path.dirname(this_dir)          # .../bouzecode
    versions_dir = os.path.dirname(pkg_root)           # .../bouzecode_versions
...
def _wikimedia_search(keywords: str, story_context: str = "") -> str | None:
    ...
    try:
        req = urllib.request.Request(api, headers={'User-Agent': 'Bouzecode/1.0'})
...
def _unsplash_search(keywords: str, width: int, height: int, access_key: str) -> str | None:
    ...
    try:
        req = urllib.request.Request(url, headers={'User-Agent': 'Bouzecode/1.0'})
        with urllib.request.urlopen(req, timeout=10) as resp:
            data = json.loads(resp.read())
