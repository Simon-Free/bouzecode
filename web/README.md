...
```powershell
bouzegui              # via entry point (after pip install -e .[web])
python -m web         # or directly
powershell -File ..\bouzegui.ps1  # one-shot launcher: ensures uv, env, deps
```
...
| `runner.py` | Spawn + track `bouzecode` subprocesses |
