...
_PLAN_HEADING_RE = __import__("re").compile(
    r"(?:^|\n)##\s*(?:Phase\s*2[:\s]|Plan\b)",
    __import__("re").IGNORECASE,
)
_PLAN_MARKER_RE = __import__("re").compile(
...
def extract_plan_content(session_path: str) -> str | None:
    ...
    # 1. Prefer explicit WritePlan tool calls (collect ALL plans)
    plans = []
    ...
    for msg in data.get("messages", []):
        ...
        if heading_match or marker_match:
            # Extract from the earliest match to end of text
            start = min(
...
def render_session_file(session_path: str, finished: bool = True) -> str | None:
    ...
    meta = {k: data.get(k) for k in (
        "session_id", "saved_at", "turn_count", "first_message",
        "total_input_tokens", "total_output_tokens",
        "total_cache_read_tokens", "total_cache_creation_tokens",
    )}
    blocks = parse_session_json(messages)
    return render_html(blocks, finished=finished, meta=meta)
...
def render_edited_files(session_path: str) -> str | None:
    ...
    if not snapshots:
        return _files_empty_page()
    return _build_files_html(snapshots)
...
def _build_files_html(snapshots: dict) -> str:
    ...
    for fp in sorted_paths:
        ...
        parts.append(
            ...
            f'<span class="badge {badge_cls}">{badge_text}</span> {_esc(fp)}'
            f'</a></li>'
        )
    parts.append('</ul>')
    ...
    for idx, fp in enumerate(sorted_paths):
        ...
        parts.append(
            ...
            f'{_esc(fp)}<span class="chevron">&#9654;</span></summary>'
