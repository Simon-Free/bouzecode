# [desc] File-based memory storage, indexing, and CRUD operations with user and project scopes. [/desc]
"""File-based memory storage with user-level and project-level scopes.

Storage layout:
  user scope    : ~/.bouzecode/memory/<slug>.md
  project scope : .bouzecode/memory/<slug>.md  (relative to cwd)
...
USER_MEMORY_DIR = Path.home() / ".bouzecode" / "memory"
...
def get_project_memory_dir() -> Path:
    ...
    return Path.cwd() / ".bouzecode" / "memory"
...
def get_memory_dir(scope: str = "user") -> Path:
    ...
    Args:
        scope: "user" (global ~/.bouzecode/memory) or
               "project" (.bouzecode/memory relative to cwd)
    """
    if scope == "project":
