# [desc] Unified /info command showing session stats, token usage, cost, and timing metrics. [/desc]
"""Unified information command: /info = context + cost + timing."""
...
def cmd_info(_args: str, state, config) -> bool:
    """Show unified session information: context, cost, and timing."""
    ...
    from bouzecode.backend.core.config import calc_cost
    ...
    # Header
    print()
    print(clr("  \u2500\u2500 Session Info \u2500\u2500", "bold"))
    print(f"  Version:     {VERSION}")
    print(f"  Model:       {model} ({provider})")
    print(f"  Permissions: {perm_mode}")
    print(f"  Session:     {session_id}  ({turn_count} turns, {msg_count} messages)")

    # Tokens
    print()
    print(clr("  \u2500\u2500 Tokens \u2500\u2500", "bold"))
    print(f"  Input (cumulated):   {tokens_in:,}")
    print(f"    cache read:        {cache_read:,}")
    print(f"    cache write:       {cache_create:,}")
    print(f"  Output:              {tokens_out:,}")
    print(f"  Context fill:        ~{est_ctx:,} / {ctx_limit:,} ({ctx_pct:.0f}%)")
    print(f"  Est. cost:           \u20ac{cost:.4f}")

    # Timing
    entries = getattr(state, "timing_entries", [])
    if entries:
        ...
        print()
        print(clr("  \u2500\u2500 Timing \u2500\u2500", "bold"))
        print(f"  Wall time:           {_fmt_duration(wall)}")
        if llm_count:
            avg_ttft = _fmt_duration(ttft_sum / llm_count)
...
def cmd_cost(_args: str, state, config) -> bool:
    ...
    cache_read = getattr(state, "total_cache_read_tokens", 0)
    cache_create = getattr(state, "total_cache_creation_tokens", 0)
    cumulated_in = state.total_input_tokens
