...
def _build_session_data(state, session_id: str | None = None, model: str | None = None) -> dict:
    ...
    return {
        ...
        # Attribution : profil effectif du run et rôle du run (work/validation/merge).
        # Champs ADDITIFS — une session écrite avant leur existence n'a pas la clé, et
        # tout lecteur (history.json, cost_dashboard) la lit avec un défaut.
        "profile": getattr(state, "profile", ""),
        "run_kind": getattr(state, "run_kind", ""),
        "close_reason": getattr(state, "close_reason", ""),
        "final_answer": getattr(state, "final_answer", ""),
        "recap": getattr(state, "recap", None),
        "recap_missing": getattr(state, "recap_missing", False),
    }
...
def _save_session_checkpoint(state, session_file: str, session_id: str | None = None, session_path: str | None = None, model: str | None = None) -> None:
    data = _build_session_data(state, session_id=session_id, model=model)
    if session_path:
