...
def cmd_plan(args: str, state, config) -> bool:
    ...
    if arg == "done":
        ...
        from bouzecode.backend.tools.plan_mode_schema import restore_write_tools
    ...
    if arg == "status":
        if in_plan_mode:
            ...
            info("Write tools hidden; the plan is written with WritePlan. Use /plan done to exit.")
        else:
    ...
    from bouzecode.backend.tools.plan_mode_schema import hide_write_tools
    hide_write_tools(config)

    info("Plan mode activated (read-only, the plan is written with WritePlan).")
