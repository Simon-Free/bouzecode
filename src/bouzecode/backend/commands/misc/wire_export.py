...
notes are surfaced separately here (from ``context_state.notes``); the methodology
...
def _load_turns(path: Path) -> dict:
    ...
    Le payload complet est une VUE : le journal ne stocke que ce qui change d'un
    enregistrement au suivant, et `payload_dump.load_turn_map` le replie — d'où le
    dossier passé plutôt que le fichier, la règle du dernier gagnant étant tenue là-bas."""
    from ...core.payload_view import load_turn_map
    return load_turn_map("", payloads_dir=path.parent)
...
def _render_turn(rec: dict) -> str:
    ...
    notes = (rec.get("context_state") or rec.get("context_state") or {}).get("notes") or {}
