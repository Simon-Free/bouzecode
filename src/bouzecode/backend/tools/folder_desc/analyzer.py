...
def _call_llm_for_description(
    ...
    used by the main chat applies here — avoids bypassing the SNCF socle
    handling that lives in `providers/backends/anthropic_stream.py`.
    """
    from ...agent.providers import stream, AssistantTurn
    ...
    try:
        ...
        # One file's description is auxiliary enrichment — if the LLM call fails
        # (no creds, network, hermetic test guard), degrade to no description
        # rather than failing the whole folder listing.
        return None
...
def _collect_code_files(root: Path) -> list[Path]:
    venv_dirs = {p.parent for p in root.rglob("pyvenv.cfg")}
    candidates = []
    for p in sorted(root.rglob("*")):
        ...
        if any(v in p.parents for v in venv_dirs):
            continue
        candidates.append(p)
    ignored = _batch_is_ignored(candidates, root)
...
def _analyze_files(files: list[Path], root: Path, config: dict) -> str:
    """Generate descriptions only for the given files (no re-collection)."""
    if not files:
    ...
    with ThreadPoolExecutor(max_workers=20) as pool:
        futures = {pool.submit(_process, fp): fp for fp in files}
        progress = tqdm(
            as_completed(futures),
            total=len(files),
            desc=f"Generating descriptions ({root.name})",
            unit="file",
            ncols=80,
            file=sys.stderr,
            leave=False,
    ...
    parts = [f"{added}/{len(files)} descriptions generated"]
    if errors:
