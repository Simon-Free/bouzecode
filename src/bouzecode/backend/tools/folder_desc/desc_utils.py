...
def _batch_is_ignored(paths: list[Path], root: Path) -> set[Path]:
    """Return subset of paths that are git-ignored, using a single subprocess."""
    if not paths:
        return set()
    always_skipped = set()
    to_check = []
    for p in paths:
        if any(part in _ALWAYS_SKIP or part.endswith('.egg-info') for part in p.parts):
            always_skipped.add(p)
        else:
            to_check.append(p)
    if not to_check:
        return always_skipped
    rel_map = {str(p.relative_to(root)).replace("\\", "/"): p for p in to_check}
    try:
        r = subprocess.run(
        ...
        git_lines = []
    result = always_skipped.copy()
    for line in git_lines:
        normalized = line.strip().replace("\\", "/")
        if normalized in rel_map:
            result.add(rel_map[normalized])
    return result
