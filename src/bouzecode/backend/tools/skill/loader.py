...
def _get_skill_paths() -> list[tuple[Path, str]]:
    ...
    base = [
        (Path.cwd() / ".bouzecode" / "skills", "project"),
        (Path.home() / ".bouzecode" / "skills", "user"),
...
def load_skills(include_builtins: bool = True) -> list[SkillDef]:
    ...
    # _get_skill_paths() returns highest-priority first → reverse to load lowest first.
    for skill_dir, src in reversed(_get_skill_paths()):
