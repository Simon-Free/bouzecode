...
from .ops.scratch import cleanup_scratch
...
def read_text_raw(path: Path) -> str:
    """Read a file without newline translation, compatibly across Python versions.

    ``Path.read_text(newline=...)`` only exists on 3.13+; ``Path.open(newline=...)``
    works everywhere. ``newline=""`` keeps CRLF/CR intact (no \\r\\n -> \\n rewrite).
    """
...
def _track_read(file_path: str, content: str | None = None) -> None:
    ...
    if content is None:
        try:
            content = read_text_raw(Path(file_path))
        except OSError:
            return
...
def _stale_edit_warning(file_path: str) -> str | None:
    ...
    try:
        current = read_text_raw(Path(file_path))
    except OSError:
        return None
