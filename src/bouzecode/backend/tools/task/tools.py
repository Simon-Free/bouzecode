# [desc] Defines and registers task management tools (create, update, get, list) with their JSON schemas. [/desc]
...
def _register() -> None:
    defs = [
        ToolDef(
            ...
            read_only=False,
            # Mutates the shared store and assigns IDs from existing tasks —
            # must run sequentially so same-batch creates get batch-order IDs.
        ...
        ToolDef(
            ...
            read_only=False,
            # Mutates the shared store (status/edges) — keep sequential.
            concurrent_safe=False,
        ),
        ToolDef(
            name="TaskGet",
