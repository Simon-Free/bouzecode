...
def _enter_plan_mode(params: dict, config: dict) -> str:
    ...
    from .plan_mode_schema import hide_write_tools
    hide_write_tools(config)

    return (
        f"Plan mode activated. You are now in read-only mode.\n"
        f"Plan file: {plan_path}\n\n"
        f"Instructions:\n"
        f"1. Analyze the codebase with your read tools (Read, Glob, Grep).\n"
        f"2. Write the plan with WritePlan \u2014 the file-writing tools are hidden "
        f"until you leave plan mode, so do not look for them.\n"
...
def _write_plan(params: dict, config: dict) -> str:
    ...
    # Dispatcher/manager profiles (plan_mode: false) opt out of the validation
    # pause — they plan by delegating, they never block on awaiting_plan_validation.
    if config.get("_plan_mode_disabled"):
        _persist_plan(content, config, plan_path)
        return f"Plan saved to {plan_path}"

    # Auto-validate the plan (skip when the user validates it manually below).
    ...
    if user_validation_required:
        ...
        if is_web_ipc_active():
            ...
            ipc_dir = config.get("_web_agent_dir")
            if ipc_dir:
                from bouzecode.web_v2.runtime import ipc as _ipc
                _ipc.write_state(
...
def _exit_plan_mode(params: dict, config: dict) -> str:
    ...
    from .plan_mode_schema import restore_write_tools
    restore_write_tools(config)
