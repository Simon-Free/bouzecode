...
def _charger_tqdm():
    """`tqdm` à l'usage, pas à l'import.

    Mesuré : 34 ms au démarrage de CHAQUE agent, pour une barre de progression que seul
    `RunPythonTest` affiche — un outil que la plupart des agents n'appellent jamais. Ce module
    est tiré par la chaîne d'imports du harnais, donc tout le monde payait."""
    try:
    ...
    try:
        for raw_line in proc.stdout:
            ...
            if total is None:
                ...
                if m:
                    total = int(m.group(1))
                    _tqdm = _charger_tqdm() if total > 0 else None
                    if _tqdm:
