...
def _spawn_terminal_agent(params: dict, config: dict) -> str:
    ...
    # Poll for result file (wait mode)
    timeout = 300
    poll_interval = 2
    elapsed = 0
    ...
    if result_path.exists():
        ...
        exit_code = proc.poll()
        task.status = "failed"
        task.result = f"No result file produced (process exit code: {exit_code})"
        result = task.result
...
def _agent_tool(params: dict, config: dict) -> str:
    ...
    When running in a terminal (not BouzéqUI), spawns in a new terminal window.
    """
    # Auto-dispatch: terminal mode → new terminal window
...
def _list_agent_types(params: dict, config: dict) -> str:
    from .definitions import load_agent_definitions
    ...
    lines.append("")
    lines.append(
        "Create custom agents: place .md files in ~/.bouzecode/agents/ or .bouzecode/agents/"
    )
    return "\n".join(lines)
...
register_tool(ToolDef(
    ...
    schema={
        ...
        "description": (
            ...
            "types (coder, reviewer, researcher, tester, or custom from .bouzecode/agents/), "
