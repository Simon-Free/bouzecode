...
class SubAgentManager:
    ...
    def _apply_profile(self, profile, config: dict, base_prompt: str) -> str:
        ...
        # Profiles that require a recap expect the STRUCTURED recap object (not the
        # legacy 6-section markdown). Coding sessions require all 4 recap fields.
        config["recap_expects_object"] = config["require_recap"]
        config["recap_coding"] = True
        allowed = list(profile.tools) if getattr(profile, "tools", None) else None
        if getattr(profile, "requires_plugins", None):
            from .plugin_resolver import ensure_plugins
            plugin_tools, errors = ensure_plugins(list(profile.requires_plugins))
            if plugin_tools:
                allowed = (allowed or []) + [t for t in plugin_tools if t not in (allowed or [])]
            if errors:
                config["_plugin_errors"] = errors
        if allowed is not None:
            config["_allowed_tools"] = allowed
        if getattr(profile, "skills", None):
            config["_profile_skills"] = list(profile.skills)
        # Expose the CURRENT profile name so tools (e.g. Agent) can enforce
        # role-based rules — notably: a manager may only spawn specialized types.
        config["_agent_type"] = profile.name
    ...
    def _resolve_profiles(self, names: list):
        """Resolve profiles named *names* from the global ~/.bouzecode, the cwd project,
        registered extra dirs and bouzecode builtins, merge them in order, and return the
        merged AgentProfile (or None if none of the names matched)."""
        from ..profiles import load_all_profiles, merge_profiles
    ...
    def spawn(
        ...
        if profile is not None:
            eff_system = self._apply_profile(profile, eff_config, eff_system)
            task.agent_type = profile.name
