...
def load_payload_dump(session_id: str) -> list[dict]:
    ...
    """
    # Le payload de chaque tour est RECONSTITUÉ : le journal ne garde que ce qui change d'un
    # appel au suivant (cf. `payload_dump`). `load_turn_records` rend des enregistrements dont
    # `messages` est complet, donc `analyze_turn_segments` travaille comme avant.
    # Le dossier est passé EXPLICITEMENT, à partir du `CONFIG_DIR` de CE module : c'est lui
    # que les appelants (et les tests) redirigent. Laisser la vue résoudre le chemin
    # elle-même déplacerait silencieusement cette couture vers `core.config`.
    from ..core.payload_view import load_turn_records
    records = load_turn_records(
        session_id, payloads_dir=Path(CONFIG_DIR) / "debug_payloads" / session_id)
