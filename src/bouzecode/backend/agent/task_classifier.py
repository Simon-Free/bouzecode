...
def classify(user_message: str, config: dict) -> dict:
    ...
    doute→escalade). Gated by config['task_classification'] (OFF by default;
    opt-in). Any failure returns {'type': 'autre', 'scope': 'doute'} with a
    warning log."""
    global dispatch_stream

    # Gate: disabled by config (default) or non-zero depth
    if not config.get("task_classification", False):
