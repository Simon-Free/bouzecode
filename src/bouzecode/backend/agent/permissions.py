...
def _propagate_denials(
    ...
    """Cancel every call that (transitively) depends on a denied one.

    `depends_on` reaches us in whatever shape the model wrote it — the XML parser
    hands over the raw string, so `depends_on=["w1"]` and `depends_on="["w1"]"`
    both arrive as the literal text `["w1"]`. Reading that as ONE alias named
    `["w1"]` matched nothing and let dependents of a denied call run anyway. The
    DAG builder already parses every accepted shape via `_coerce_list`; reuse it
    so the two readings of the same field can never drift apart.
    """
    ...
    for tc in all_tcs:
        raw = _coerce_list(tc["input"].get("depends_on"))
