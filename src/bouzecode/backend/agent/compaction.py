# [desc] Two-layer context window compression: truncates old tool results and LLM-summarizes old messages. [/desc]
"""Context window management: two-layer compression for long conversations."""
from __future__ import annotations

from . import providers
from ..core._embedded_data import COMPACTION_SYSTEM_PROMPT
...
def get_context_limit(model: str) -> int:
    ...
    """
    raw = (model.split("/", 1)[1] if "/" in model else model).lower()
    if raw.startswith("claude"):
        return 200000
    if raw.startswith("gemini") or raw.startswith("qwen"):
        return 1000000
    if raw.startswith("deepseek-chat"):
        return 64000
    # gpt-*, o1/o3, ollama, deepseek-v4, and unknown models → 128000
    return 128000
...
def maybe_compact(state, config: dict) -> bool:
    ...
    est2 = estimate_tokens(state.messages)
    entry["est_tokens_after_snip"] = est2

    if est2 <= threshold:
        entry["result"] = "snip_sufficient"
    ...
    est3 = estimate_tokens(state.messages)
    entry["est_tokens_after_compact"] = est3
    entry["message_count_after"] = len(state.messages)
    entry["result"] = "full_compact"
    state.compaction_log.append(entry)
    return True
