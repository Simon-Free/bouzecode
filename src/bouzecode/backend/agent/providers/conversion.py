...
def messages_to_anthropic(messages: list, cache_last: bool = True, meth_delta: str = "") -> list:
    ...
    while i < len(messages):
        ...
        if role == "user":
            ...
            i += 1
            # Gather tool results into a single user message. Image tool_results
            # (produced by Read on an image file, sentinel "__BOUZE_IMAGE__:<mime>:<b64>")
            # become native Anthropic image blocks so the model can actually SEE them.
            tool_xmls = []
            image_blocks = []
            while i < len(messages) and messages[i].get("role") == "tool":
                tm = messages[i]
                tm_content = tm.get("content", "") or ""
                if tm_content.startswith("__BOUZE_IMAGE__:"):
                    _, media_type, b64 = tm_content.split(":", 2)
                    image_blocks.append({
                        "type": "image",
