# [desc] Streams from OpenRouter via requests, through the outbound proxy if any, with XML tool parsing. [/desc]
...
def stream_openrouter(
    ...
    """Stream from an OpenAI-compatible endpoint (OpenRouter by default, or any
    other gateway via the base_url/session_factory overrides). Tool calls are
    parsed from XML in the text stream (see xml_tool_protocol/) — same protocol
    as the Anthropic backend, so the rest of the agent loop is provider-agnostic.

    send_reasoning=False omits the OpenRouter-specific `reasoning` param, which
    plain OpenAI-compatible gateways reject with HTTP 400. reasoning_effort sets
    the OpenAI `reasoning_effort` (e.g. "low") for GPT-5 reasoning models.
    provider_label names the backend in logs/errors (other gateways reuse this
    streamer, so the label must not always say OpenRouter)."""
    import urllib3
    ...
    if send_reasoning and config.get("native_reasoning", False):
    ...
    # Resolve _build_session as a module global at call time (not as a default
    # arg) so tests that monkeypatch openrouter_stream._build_session take effect.
    session = (session_factory or _build_session)()
    started = False
    for attempt in range(_EMPTY_COMPLETION_RETRIES + 1):
        ...
        if attempt < _EMPTY_COMPLETION_RETRIES:
            logger.warning(
                "%s returned an empty completion (finish=%s); retrying %d/%d",
                provider_label, stop_reason, attempt + 1, _EMPTY_COMPLETION_RETRIES,
            )
