...
from bouzecode.backend.tools.interaction import DeferredChecks, PausedForInput
...
def _build_turn_generator(user_input: str, state, config: dict, system_prompt: str):
    ...
    if config.pop("_resume_pending", False):
        from ..web_v2.runtime import pending as web_pending
    ...
    error_log = config.pop("_resume_deferred", "")
    if error_log:
        from ..web_v2.runtime import deferred as web_deferred
        sf = config.get("_session_file")
        if sf and web_deferred.exists(sf):
            web_deferred.delete(sf)
        user_input = f"[SYSTEM] Le check differe a echoue: {error_log}. Corrige puis re-emets FinalAnswer."
    return _run(user_input, state, config, system_prompt, cancel_check=cancel_check)


def _web_keep_warm(config) -> bool:
    """True when running as a web agent that should stay warm (process alive) between turns.

    Un SOUS-AGENT ne reste PAS chaud. Le quart d'heure de résidence existe pour laisser à
    l'utilisateur le temps de revenir sur sa conversation ; un sous-agent n'a pas
    d'utilisateur — seul son manager peut lui réécrire, et il le fait rarement. Garder les
    douze sous-agents d'un manager résidents coûte douze process (89 à 687 Mo chacun) qui
    tiennent leur slot de warm-pool au détriment des conversations où quelqu'un reviendra.

    La parenté vient de l'ENV (`BOUZECODE_PARENT`, posé par `runner._ticket_env`) : le
    process ne la connaît pas autrement, seul le serveur la connaît. Doublé par une règle
    d'éviction côté serveur (`warmpool.decide_evictions`) — une politique qui ne tient qu'à
    un seul point de câblage cesse de s'appliquer en silence le jour où ce point change."""
    return bool(config.get("_web_agent_dir")) and not os.environ.get("BOUZECODE_PARENT")


def flush_thinking(chunks: list[str], turn: int, thinking_log: list) -> str:
    """Écrit le raisonnement DE CE TOUR dans le journal, VIDE l'accumulateur, et le renvoie.

    LE BUG QUE CE VIDAGE CORRIGE (mesuré le 2026-07-30). L'accumulateur est créé UNE FOIS par
    message utilisateur et alimenté à chaque morceau de raisonnement ; il n'était jamais
    réinitialisé, alors que le journal reçoit une entrée à CHAQUE fin de tour. Chaque entrée
    réécrivait donc tout l'historique précédent, et le champ grossissait en O(tours²). Sur une
    session réelle de 270 tours : 259 entrées sur 259 étaient un préfixe strict de la suivante,
    649 Ko de texte unique occupaient 109 Mo — un facteur 171. C'est la première cause des JSON
    de session à 113 Mo, donc des 55 s de décodage que `web_v2/services/sessions/meta_index.py`
    documente sur un seul `GET /api/sessions`.

    POURQUOI CETTE FONCTION EXISTE, plutôt que trois lignes dans la boucle : c'est le seul moyen
    d'en faire une garantie TESTABLE. Le harnais de tests (`tests/e2e_harness.py`) importe
    `backend.agent.loop.run` et ne passe JAMAIS par ce module — aucune conversation scriptée ne
    peut donc atteindre le journal, ce qui explique qu'un bug d'un facteur 171 ait vécu sans
    qu'aucun test ne bronche."""
    text = "".join(chunks)
    if text:
        thinking_log.append({"turn": turn, "text": text})
        chunks.clear()
    return text


def _resume_paused_warm(pause: PausedForInput, state, config, system_prompt, cancel_check) -> None:
    """On AskUserQuestion under a WARM web agent: persist a COLD fallback (so a crash while
    idle is still resumable), then stay alive polling answer.txt. When the UI answers, resume
    the paused turn IN-PROCESS (no cold-start respawn) and loop if it pauses again.

    Falls back to _persist_pause_and_exit semantics on disk (checkpoint + pending) BEFORE the
    idle wait, so nothing is lost if the process dies while waiting."""
    import time as _time
    from bouzecode.backend.agent.loop import resume_paused
    from ..web_v2.runtime import pending as web_pending
    from ..web_v2.runtime import ipc as web_ipc
    ...
    current_pause = pause
    while True:
        # COLD fallback on disk: if we die while idle, the agent is still resumable.
        if sf:
            _save_session_checkpoint(state, sf, config.get("_session_id"), config.get("_session_path"), model=config.get("model"))
            save_progressive(state, config)
            web_pending.save(sf, current_pause)
        if paths is not None:
            status = "awaiting_plan_validation" if current_pause.is_plan_validation else web_ipc.STATUS_AWAITING_INPUT
            web_ipc.write_state(
                paths,
                status,
                question=current_pause.question,
                options=current_pause.options,
                allow_freetext=current_pause.allow_freetext,
                turn=getattr(state, "turn_count", 0),
            )
        # No IPC channel -> behave like cold (exit). Shouldn't happen for a web agent.
        if paths is None:
            sys.exit(0)

        # Warm idle-wait: poll answer.txt until the UI responds (or cancel / TTL).
        last_activity = _time.monotonic()
        answer = None
        while True:
            if web_ipc.is_cancelled(paths) or web_ipc.consume_cancel(paths):
                web_ipc.write_state(paths, web_ipc.STATUS_FINISHED, turn=getattr(state, "turn_count", 0))
                sys.exit(0)
            answer = web_ipc.pop_text(paths.answer)
            if answer is not None:
                break
            if _time.monotonic() - last_activity > ttl_seconds:
                web_ipc.write_state(paths, web_ipc.STATUS_FINISHED, turn=getattr(state, "turn_count", 0))
                sys.exit(0)
            _time.sleep(poll_interval)

        # Answer received -> resume the paused turn in-process.
        if sf:
            web_pending.delete(sf)
        web_ipc.write_state(paths, web_ipc.STATUS_RUNNING, turn=getattr(state, "turn_count", 0))
        try:
            gen = resume_paused(current_pause, answer, state, config, system_prompt, cancel_check=cancel_check)
            for _ in gen:
                pass
        except PausedForInput as next_pause:
            current_pause = next_pause
            continue  # pauses again -> re-persist + idle-wait
        # Turn finished cleanly -> back to normal warm idle handled by the caller loop.
        return
...
def _persist_deferred_and_exit(deferred: DeferredChecks, state, config) -> None:
    """On FinalAnswer with a non-empty deferred_queue: save session, persist the
    deferred checks (answer + checks), then exit. The runner drains the checks
    after the subprocess exits and only respawns the model if one fails."""
    sf = config.get("_session_file")
    if sf:
        _save_session_checkpoint(state, sf, config.get("_session_id"), config.get("_session_path"), model=config.get("model"))
        save_progressive(state, config)
        from ..web_v2.runtime import deferred as web_deferred
        web_deferred.save(sf, deferred)
    sys.exit(0)
...
def repl(config: dict, initial_prompt: str = None):
    ...
    from bouzecode.backend.tools.ops import scratch as _scratch
    _scratch.set_scratch_session(session_id)
    ...
    def run_query(user_input: str, is_background: bool = False):
        ...
        with query_lock:
            ...
            try:
                ...
                for event in gen:
                    ...
                    if isinstance(event, TextChunk):
                        ...
                        if event.tool_id not in _shown_inline_ids:
                            flush_response()
                            print_tool_start(event.name, event.inputs, verbose)
                        # The tools execute *after* the last ToolStart and block until
                        # ToolEnd, yielding nothing meanwhile — keep the spinner running
                        # so the terminal shows activity during that work.
                        _start_tool_spinner()
                        spinner_shown = True
                        ...
                        if thinking_started:
                            ...
                            print()
                            thinking_started = False
                        _turn_thinking = flush_thinking(
                            _thinking_chunks, state.turn_count, state.thinking_log)
                        sf = config.get("_session_file")
                        if sf:
                            _save_session_checkpoint(state, sf, config.get("_session_id"), config.get("_session_path"), model=config.get("model"))
                        ...
                        if last_llm and last_llm.get("duration", 0.0) > 0.05:
                            ...
                            if thinking > 0.05:
                                ...
                                # Longueur du raisonnement DE CE TOUR (l'accumulateur vient
                                # d'être vidé juste au-dessus). Avant, cette ligne sommait
                                # l'accumulateur cumulé : la mention « (N K chars) » du bandeau
                                # de timing annonçait donc le total de la session, pas le tour.
                                _tc = len(_turn_thinking)
                                if _tc > 0:
                ...
                _stop_tool_spinner()
                flush_response()
                if _web_keep_warm(config):
                    _resume_paused_warm(pause, state, config, system_prompt, config.get("_cancel_check"))
                else:
                    _persist_pause_and_exit(pause, state, config)
            except DeferredChecks as deferred:
                _stop_tool_spinner()
                flush_response()
                _persist_deferred_and_exit(deferred, state, config)
            except Exception as e:
                _stop_tool_spinner()
                # Fatal provider error (marked by loop.run): persist the checkpoint
                # so the disk session carries close_reason='api_error' before the
                # process dies. Without this re-save, the last checkpoint predates
                # the error and web_v2 sees a session with no death cause.
                if getattr(state, "close_reason", "") == "api_error":
                    sf = config.get("_session_file")
                    if sf:
                        _save_session_checkpoint(
                            state, sf, config.get("_session_id"),
                            config.get("_session_path"), model=config.get("model"),
                        )
                raise e
    ...
    if resume_path and Path(resume_path).exists():
        ...
        state.compaction_log = _resumed.get("compaction_log", [])
    ...
    if initial_prompt or resume_auto:
        ...
        if web_agent_dir:
            from ..web_v2.runtime import ipc as _ipc
            _ipc_paths = _ipc.from_dir(web_agent_dir)
            config["_cancel_check"] = lambda: _ipc.consume_cancel(_ipc_paths)
            # Même demande, lue SANS la consommer, pour les points d'interruption qui ne
            # closent pas le tour eux-mêmes — aujourd'hui les frontières d'outils (`dag`).
            # Le lot d'outils est le gros du temps d'un tour et ne regardait rien : une
            # interruption web y restait lettre morte jusqu'à ce que l'escalade tue le
            # process. Consommer là-bas volerait la demande à la garde de tête de
            # `loop.run`, seule à savoir clore proprement (`close_reason='cancelled'`).
            config["_cancel_peek"] = lambda: _ipc.is_cancelled(_ipc_paths)
            # Stamp the IPC per-turn (turn=N, not the hardcoded turn=1) and — on a
            # graceful close — write FINISHED + close_reason BEFORE _fire_completion's
            # on_completion hook runs. That hook http_posts the test-gate synchronously
            # (timeout 1900s) and the server can reap/kill this pid inside that call,
            # so control may never return to run_agent_event_loop's FINISHED write.
            # Without the finish cb the IPC stays {running, turn=1} -> reconcile_dead_agents
            # sees a dead pid + running IPC and reports a false crash (rc=-1).
            config["_ipc_turn_cb"] = lambda t: _ipc.write_state(
                _ipc_paths, _ipc.STATUS_RUNNING, turn=t
            )
            # Battement d'ACTIVITÉ : les outils qui démarrent, horodatés par write_state.
            # Sans lui, l'IPC ne bougeait qu'au changement de tour — donc rien pendant les
            # minutes d'exécution des outils, seul moment où l'utilisateur se demande
            # justement ce que fait l'agent. Le statut reste RUNNING : c'est bien ce que
            # l'agent est, on ne fait qu'ajouter DE QUOI il est en train de s'occuper.
            config["_ipc_activity_cb"] = lambda tools: _ipc.write_state(
                _ipc_paths, _ipc.STATUS_RUNNING, turn=state.turn_count, tools=tools
            )
            config["_ipc_finish_cb"] = lambda cr: _ipc.write_state(
                _ipc_paths, _ipc.STATUS_FINISHED, turn=state.turn_count, close_reason=cr
            )
