...
def _strip_tool_xml(texte: str) -> str:
    """Import PARESSEUX du renderer web.

    Mesuré : 30 ms au démarrage de chaque agent. `bouzecode.ui.__init__` importe ce module,
    qui tirait tout `web_v2.runtime.html_renderer` — le rendu HTML du SERVEUR — juste pour
    nettoyer du XML au moment de rejouer une session. Un agent en ligne de commande ne rejoue
    presque jamais, et n'a rien à faire du renderer web."""
    from bouzecode.web_v2.runtime.html_renderer import strip_tool_xml
    return strip_tool_xml(texte)
...
def replay_messages(messages: Iterable[dict]) -> None:
    ...
    for m in msgs:
        ...
        if role == "user":
            ...
            text = _strip_tool_xml(raw_text) if tool_calls else raw_text
            if text:
                stream_text(text)
                flush_response()
            for tc in tool_calls:
