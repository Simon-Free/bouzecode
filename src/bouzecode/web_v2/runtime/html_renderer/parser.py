...
Block = UserMessage | AssistantText | ToolCall | ToolResult | SystemNotice
...
def parse_session(text: str) -> list[Block]:
    ...
    # Find thinking block ranges to exclude from tool parsing
    thinking_ranges = [(m.start(), m.end()) for m in _THINKING_BLOCK_RE.finditer(text)]

    def _in_thinking(pos: int) -> bool:
        return any(s <= pos < e for s, e in thinking_ranges)

    markers: list[tuple[int, int, str, re.Match]] = []
    for m in _TOOL_USE_RE.finditer(text):
        if not _in_thinking(m.start()):
