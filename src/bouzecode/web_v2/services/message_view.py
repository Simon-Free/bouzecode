...
def _assistant_block(message: dict, context_url: str | None = None) -> str:
    ...
    if context_url is not None:
        ...
        parts.append(
            ...
            f'title="Détail du contexte envoyé au modèle à ce tour '
            f'(statut cache + tour d\'ajout par élément)">?</a>'
        )
    ...
    if tool_calls:
        n = len(tool_calls)
        label = f"{n} outil" if n == 1 else f"{n} outils"
        parts.append(f'<details class="tools-group"><summary>{label}</summary>')
...
def _tool_call_html(tool_call: dict) -> str:
    ...
    return (
        f'<details class="tc {css_class} pui-tool-panel"{id_attr}><summary><span class="tc-kind">{label}</span> <span class="pui-dot"></span> '
...
def _subagent_event_block(message: dict) -> str:
    ...
    if subtype == "done":
        ...
        suffix = f" — verdict {html.escape(verdict)}" if verdict else ""
        ...
        return (
            ...
            f'<span class="subagent-event-line">✅ {label}{_event_time(a)} terminé{suffix}</span></div>'
        )
    # launch
    if count > 1:
        items = "".join(
        ...
        return (
            ...
            f'<summary class="subagent-event-line">🤖 {count} agents lancés</summary>'
    ...
    return (
        ...
        f'data-open-key="{html.escape(a.get("open_key", ""))}">'
        f'<span class="subagent-event-line">🤖 1 agent lancé — {label}{_event_time(a)}</span></div>'
    )
...
def _tool_result_block(message: dict) -> str:
    ...
    if kind == "final_answer":
        ...
        return (
            ...
            f'<div class="role pui-eyebrow">✅ Réponse finale</div>'
            f'{render_markdown(answer_text)}</div>'
        )
    if kind == "final_answer_refused":
        return (
            f'<div class="block final-answer-refused">'
            f'<div class="role pui-eyebrow">❌ Clôture refusée par le validateur</div>'
    ...
    return (
        f'<details class="tr{css_class} pui-tool-panel"{id_attr}><summary><span class="pui-dot"></span> {html.escape(header)}</summary>'
