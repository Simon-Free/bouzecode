...
// désormais une phrase (`activity_label`), la phase d'un lancement (`phase_label`) et un
// drapeau de silence anormal (`stale`) : ces tests prouvent que la carte les MONTRE.
...
const NODE_AU_TRAVAIL = {
  ...
  activity: "Bash", activity_label: "Bash en cours depuis 3 min", idle_seconds: 180,
  turn: 9, stale: false,
};
...
describe("une carte dit ce que l'agent est en train de faire", () => {
  ...
  it("un silence anormal est signalé PAR DU TEXTE, jamais par un point seul", async () => {
    ...
    expect(chip.title).toContain("battement");
    expect(chip.title).toContain("pas un constat de mort");
  });
  ...
  it("un ticket en préparation dit « préparation… » et l'étape en cours", async () => {
    ...
    expect(row.querySelector(".badge").title).toContain("préparation…");
    expect(row.title).toContain("préparation…");
    ...
    expect(chip.textContent).toBe("création du worktree — depuis develop");
  });
