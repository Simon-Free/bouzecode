# [desc] File-based IPC contract between BouzéqUI and the bouzecode agent subprocess (state, followup, answer, cancel). [/desc]
...
def run_agent_event_loop(initial_prompt, run_query, paths: IPCPaths) -> None:
    ...
    BouzéqUI drives multi-turn conversations by respawning a new subprocess per
    ...
    try:
        ...
        return
    # Don't overwrite if the tool already set an awaiting state (e.g. WritePlan validation)
    current = read_state(paths)
    if current.get("status") not in ("awaiting_plan_validation", "awaiting_input"):
