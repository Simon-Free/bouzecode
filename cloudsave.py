# [desc] Cloud session sync via GitHub Gist: upload, list, and download bouzecode sessions. [/desc]
"""
Cloud sync for bouzecode sessions via GitHub Gist.
...
Config keys (stored in ~/.bouzecode/config.json):
...
def upload_session(
    ...
    filename = f"bouzecode_session_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def list_sessions(token: str, max_results: int = 20) -> tuple[list[dict], str | None]:
    """
    List Gists tagged as bouzecode sessions.
