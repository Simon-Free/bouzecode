...
from ui.ansi import clr, info, ok
...
def process_sentinel_result(result, state, config, run_query, track_ctrl_c) -> None:
    ...
    while isinstance(result, tuple):
        ...
        if result[0] == "__brainstorm__":
            ...
            ok_run = _safe_run(brain_prompt, run_query, track_ctrl_c)
            if ok_run:
                ...
                _safe_run(
                    f"Based on the Master Plan you just synthesized, generate a todo list file at {todo_path}. "
                ...
                info(f"TODO list saved to {todo_path}. Edit it freely, then use /worker to start implementing.")
            if from_ssj:
                result = handle_slash("/ssj", state, config)
                continue
            return
        ...
        if result[0] == "__ssj_promote_worker__":
            ...
            promote_prompt = (
                ...
                "Convert each idea into a task with checkbox format (- [ ] task description). "
                f"Write them to {todo_path_str} using the Write tool. Prioritize by impact. "
                "Do NOT explain, just write the file now."
            )
            print(clr(f"\n  \u2500\u2500 Generating TODO list from {Path(md_path).name} \u2500\u2500", "dim"))
            ok_run = _safe_run(promote_prompt, run_query, track_ctrl_c)
            if not ok_run:
                result = handle_slash("/ssj", state, config)
                continue
