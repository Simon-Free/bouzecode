# [desc] Parses markdown files with YAML frontmatter into SkillDef dataclass objects for skill loading. [/desc]
...
def _get_skill_paths() -> list[Path]:
    return [
        ...
        Path.home() / ".claude" / "skills",         # Claude Code skills (subdir/skill.md format)
    ]
...
def load_skills(include_builtins: bool = True) -> list[SkillDef]:
    ...
    # Lowest-priority paths first so higher-priority entries overwrite them.
    # Order in `_get_skill_paths()`: project, user, claude  → reverse so we load claude first.
    skill_paths = _get_skill_paths()
    sources = {0: "project", 1: "user", 2: "claude"}
    for original_index, skill_dir in reversed(list(enumerate(skill_paths))):
        src = sources.get(original_index, "user")
