# [desc] Implements `/status` and `/doctor` CLI commands for runtime diagnostics and environment checks. [/desc]
"""Diagnostic commands: status, doctor."""
...
def cmd_status(args: str, state, config) -> bool:
    ...
    _last_llm = next(
        (e for e in reversed(getattr(state, "timing_entries", []))
         if e.get("phase") == "llm"), None
    )
    last_raw_in = _last_llm.get("in_tokens", 0) if _last_llm else 0
    distinct_in = getattr(state, "distinct_base", 0) + last_raw_in
    ...
    print(f"  Tokens:      {distinct_in:,} distinct in / {tokens_out:,} out / {tokens_in:,} cumulated in")
