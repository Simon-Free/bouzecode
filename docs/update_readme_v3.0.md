# Bouzecode — Update Notes

This document describes three major feature additions to bouzecode:
...
- `~/.bouzecode/agents/<name>.md` (user-level)
- `.bouzecode/agents/<name>.md` (project-level, takes priority)
...
4. `context.py` system prompt template lists Agent, SendMessage, etc. under
   `## Multi-Agent`.
5. `bouzecode.py` `/agents` command calls `get_agent_manager().list_tasks()`
...
| `bouzecode.py` | `/agents` command; background notification; `_tool_desc()` |
...
`~/.bouzecode/memory/`) and **project** (per-repo, `.bouzecode/memory/`).
...
.bouzecode/memory/   ← project-local (relative to cwd)
  MEMORY.md
  ...
```
...
4. `memory.py` (top-level) is now a shim:
   ...
   ```
5. `bouzecode.py` `/memory` command uses `scan_all_memories()` to display a
...
| `bouzecode.py` | `/memory` command uses `scan_all_memories()` |
...
Place `.md` files in:
- `~/.bouzecode/skills/<name>.md` (user-level)
- `.bouzecode/skills/<name>.md` (project-level, takes priority)
...
```
builtin  <  user (~/.bouzecode/skills/)  <  project (.bouzecode/skills/)
```
...
| `bouzecode.py` | `cmd_skills`, `handle_slash`, REPL loop updated |
...
Create `~/.bouzecode/agents/myagent.md`:
```markdown
---
name: myagent
...
`~/.bouzecode/memory/my_note.md` with frontmatter:
```markdown
---
name: my note
...
Create `~/.bouzecode/skills/myskill.md` (user-level) or
`.bouzecode/skills/myskill.md` (project-level):
```markdown
---
name: myskill
...
redirect file system paths to `tmp_path` so no real `~/.bouzecode/`
