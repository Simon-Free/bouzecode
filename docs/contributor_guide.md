...
1. `bouzecode.py` handles CLI + REPL + slash commands.
...
- REPL command wiring lives in `bouzecode.py` (`cmd_checkpoint`, `cmd_rewind`).
...
- REPL command wiring lives in `bouzecode.py` (`cmd_voice`).
...
1. Add `cmd_<name>` function in `bouzecode.py`.
...
3. Add slash command wiring in `bouzecode.py` only if user-facing command is needed.
...
- **Path naming note:** most runtime dirs use `.bouzecode` (underscore).
