...
This document is for developers who want to understand, modify, or extend bouzecode.
...
User Input
    │
    ▼
bouzecode.py  ── REPL, slash commands, rendering
    │
...
File-based memory system stored in `~/.bouzecode/memory/`.
...
**Search order:** Project-level (`./.bouzecode/skills/`) overrides user-level
(`~/.bouzecode/skills/`) when skill names collide.
...
Defaults stored in `~/.bouzecode/config.json`. Key settings:
...
11. bouzecode.py renders diff with ANSI colors (red/green)
