...
<div align=center>
<center style="color:#000000;text-decoration:underline">Telegramブリッジ：スマホからBouzecodeを操作</center>
 </div>
...
Bouzecode は、Claude Code を Python で再実装した **軽量** かつ **使いやすい** ツールで、Claude、GPT、Gemini、Kimi、Qwen、Zhipu、DeepSeek、MiniMax、さらに Ollama や OpenAI 互換エンドポイント経由のローカルなオープンソースモデルなど、**ほぼあらゆるモデル** をサポートします。
...
**Bouzecode** は同じ中核ループを読みやすい Python で再実装しています。軽量で改造しやすく、複数プロバイダに対応し、ローカル実行も可能です。必要なものを残し、不必要な複雑さを削ぎ落としています。 詳細は以下を参照してください: [English version](https://github.com/SafeRL-Lab/clawspring/blob/main/docs/comparison_claude_code_vs_nano_v3.03_en.md) and [Chinese version](https://github.com/SafeRL-Lab/clawspring/blob/main/docs/comparison_claude_code_vs_nano_v3.03_cn.md)

### ひと目でわかる比較

| 項目 | Claude Code (TypeScript) | Bouzecode (Python) |
|-----------|--------------------------|---------------------------|
...
| 項目 | OpenClaw (TypeScript) | Bouzecode (Python) |
|-----------|----------------------|---------------------|
