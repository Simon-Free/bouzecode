...
**Goal:** Evolve bouzecode into a capable AI coding CLI with context management, pluggable tools, sub-agents, memory, skills, and diff view.
...
- `bouzecode.py` — Add /memory, /skill, /agents slash commands, diff rendering
...
Run: `cd D:/git/open-cc/bouzecode && python -m pytest tests/test_tool_registry.py -v`
...
Run: `cd D:/git/open-cc/bouzecode && python -c "from tools import execute_tool; from tool_registry import get_all_tools; print(f'{len(get_all_tools())} tools registered'); print(execute_tool('Read', {'file_path': 'config.py'}, 'auto'))"`
...
## Task 3: Diff View (`tools.py` + `bouzecode.py`)

**Files:**
- Modify: `tools.py` (the `_edit` and `_write` functions)
- Modify: `bouzecode.py` (the `print_tool_end` function)
...
Run: `cd D:/git/open-cc/bouzecode && python -m pytest tests/test_diff_view.py -v`
...
- [ ] **Step 6: Add diff rendering to bouzecode.py**
...
Also ensure the color dict `C` (line 60-70 of bouzecode.py) includes the needed keys. Check and add if missing:
...
Run: `cd D:/git/open-cc/bouzecode && python -c "
...
```bash
cd D:/git/open-cc/bouzecode
git add tools.py bouzecode.py tests/test_diff_view.py
...
Run: `cd D:/git/open-cc/bouzecode && python -m pytest tests/test_compaction.py -v`
...
# compaction.py
"""
Context window management for bouzecode.
...
Run: `cd D:/git/open-cc/bouzecode && python -m pytest tests/test_memory.py -v`
...
# memory.py
"""
File-based memory system for bouzecode.
...
MEMORY_DIR = Path.home() / ".bouzecode" / "memory"
...
Run: `cd D:/git/open-cc/bouzecode && python -m pytest tests/test_subagent.py -v`
...
# subagent.py
"""
Sub-agent lifecycle management for bouzecode.
...
- Modify: `bouzecode.py` (skill dispatch in REPL)
...
Run: `cd D:/git/open-cc/bouzecode && python -m pytest tests/test_skills.py -v`
...
# skills.py
"""
Skill loading and execution for bouzecode.
...
SKILL_PATHS = [
    Path.cwd() / ".bouzecode" / "skills",
    Path.home() / ".bouzecode" / "skills",
]
...
- [ ] **Step 5: Integrate skills into bouzecode.py REPL**

In `bouzecode.py`, add import near the top:
...
def cmd_skills(_args, _state, _config):
    skills = load_skills()
    if not skills:
        info("No skills found. Place .md files in ~/.bouzecode/skills/")
        return True
    for s in skills:
...
```bash
cd D:/git/open-cc/bouzecode
git add skills.py tests/test_skills.py bouzecode.py
...
- Modify: `bouzecode.py` (add /memory, /agents commands)
...
Run: `cd D:/git/open-cc/bouzecode && python -m pytest tests/ -v`
...
```bash
cd D:/git/open-cc/bouzecode
git add config.py bouzecode.py
...
Run: `cd D:/git/open-cc/bouzecode && python -m pytest tests/ -v --tb=short`
