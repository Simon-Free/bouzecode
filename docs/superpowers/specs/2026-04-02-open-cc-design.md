# Open-CC: Bouzecode Enhancement Design

**Date:** 2026-04-02
**Status:** Approved
**Target:** GPT-5.4, Gemini 3/3.1 Pro (Claude not in scope)
**Code budget:** ~10K lines total (currently ~2.2K)
**Constraint:** PR-friendly, mergeable back to bouzecode upstream

---

## 1. Overview

Evolve bouzecode from a minimal ~2.2K-line reference implementation into a capable AI coding CLI, approaching Claude Code's core functionality while staying lean. Five enhancement areas:
...
```
bouzecode/
├── bouzecode.py      # REPL -- add /memory, /skill slash commands
...
```python
SKILL_PATHS = [
    Path.cwd() / ".bouzecode" / "skills",    # project-level (priority)
    Path.home() / ".bouzecode" / "skills",    # user-level
]
```
...
In `bouzecode.py`, unmatched `/` commands fall through to skill lookup:
...
### 8.2 REPL Rendering (in bouzecode.py)
...
| 2 | Diff view in `tools.py` + `bouzecode.py` | Phase 1 | ~100 |
...
| 6 | `skills.py` + bouzecode.py integration | Phase 1, 4 | ~200 |
