# [desc] Loads, merges, and manages MCP server configurations from user-level and project-level JSON files. [/desc]
...
Config search order (project-level overrides user-level by server name):
  1. ~/.bouzecode/mcp.json          — user-level, lowest priority
...
USER_MCP_CONFIG  = Path.home() / ".bouzecode" / "mcp.json"
