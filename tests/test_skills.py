# [desc] Unit tests for skill definition parsing, loading, argument substitution, and trigger matching. [/desc]
...
def test_load_skills_fast_testing_builtin(monkeypatch):
    """The fast-testing builtin skill is registered and exposes its testing philosophy."""
    monkeypatch.setattr(_loader, "_get_skill_paths", lambda: [])
    skills = load_skills()
    ft = next((s for s in skills if s.name == "fast-testing"), None)
    assert ft is not None, "fast-testing builtin skill must be registered"
    assert ft.source == "builtin"
    assert ft.user_invocable is True
    assert ft.context == "inline"
    assert "/fast-testing" in ft.triggers
    prompt = ft.prompt.lower()
    # Core philosophy must be encoded in the prompt so it stays authoritative.
    assert "playwright" in prompt
    assert "llm" in prompt
    assert "vitesse" in prompt or "rapide" in prompt


def test_render_profile_skills_preloads_declared_skills(monkeypatch):
    """A profile's declared skills are preloaded (bodies inlined) in the system prompt."""
    from bouzecode.backend.core.context import render_profile_skills

    monkeypatch.setattr(_loader, "_get_skill_paths", lambda: [])
    rendered = render_profile_skills(["commit"])
    # The commit skill body must be inlined, not just listed.
    assert "commit" in rendered.lower()
    assert "préchargées" in rendered or "Skills" in rendered
    # A skill not declared must not be force-loaded here.
    assert rendered  # non-empty


def test_render_profile_skills_empty_falls_back_to_discovery(monkeypatch):
    """No declared skills → generic SkillList()/Skill() discovery instruction."""
    from bouzecode.backend.core.context import render_profile_skills, get_skills_section

    monkeypatch.setattr(_loader, "_get_skill_paths", lambda: [])
    assert render_profile_skills([]) == get_skills_section()


def test_load_skills_finds_nested_skill_md(tmp_path, monkeypatch):
    """Claude Code layout: <name>/skill.md inside the skills directory."""
    skills_dir = tmp_path / "skills"
    nested = skills_dir / "conversation-download"
    nested.mkdir(parents=True)
    (nested / "skill.md").write_text(COMMIT_MD.replace("name: commit", "name: conversation-download"))
    monkeypatch.setattr(_loader, "_get_skill_paths", lambda: [skills_dir])
    monkeypatch.setattr(_loader, "_BUILTIN_SKILLS", [])
    names = {s.name for s in load_skills()}
    assert "conversation-download" in names


def test_load_skills_finds_nested_uppercase_skill(tmp_path, monkeypatch):
    """SKILL.md (uppercase) is also accepted in nested layout."""
    skills_dir = tmp_path / "skills"
    nested = skills_dir / "uppercase"
    nested.mkdir(parents=True)
    (nested / "SKILL.md").write_text(COMMIT_MD.replace("name: commit", "name: uppercase"))
    monkeypatch.setattr(_loader, "_get_skill_paths", lambda: [skills_dir])
    monkeypatch.setattr(_loader, "_BUILTIN_SKILLS", [])
    assert any(s.name == "uppercase" for s in load_skills())


def test_load_skills_flat_and_nested_coexist(tmp_path, monkeypatch):
    """Flat <name>.md and nested <name>/skill.md in the same dir both load."""
    skills_dir = tmp_path / "skills"
    skills_dir.mkdir()
    (skills_dir / "commit.md").write_text(COMMIT_MD)
    nested = skills_dir / "review-pr"
    nested.mkdir()
    (nested / "skill.md").write_text(REVIEW_MD.replace("name: review", "name: review-pr"))
    monkeypatch.setattr(_loader, "_get_skill_paths", lambda: [skills_dir])
    monkeypatch.setattr(_loader, "_BUILTIN_SKILLS", [])
    names = {s.name for s in load_skills()}
