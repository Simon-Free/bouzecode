# [desc] Tests for voice package helpers: WAV wrapping, keyterm extraction, availability, and REPL integration. [/desc]
...
class TestReplVoiceIntegration:
    def test_voice_in_commands(self):
        import bouzecode
        assert "voice" in bouzecode.COMMANDS

    def test_voice_command_callable(self):
        import bouzecode
        assert callable(bouzecode.COMMANDS["voice"])

    def test_handle_slash_voice_sentinel(self):
        """handle_slash('/voice ...') propagates __voice__ sentinel from cmd_voice."""
        import bouzecode
        from commands import voice_cmd

        sentinel = ("__voice__", "hello world")
        with patch.object(voice_cmd, "cmd_voice", return_value=sentinel):
            bouzecode.COMMANDS["voice"] = voice_cmd.cmd_voice
            result = bouzecode.handle_slash("/voice", object(), {})
    ...
    def test_voice_status_no_crash(self, capsys):
        ...
        try:
            cmd_voice("status", object(), {})
        except SystemExit:
            pass

    def test_voice_lang_set(self, capsys):
        from commands import voice_cmd
        voice_cmd.cmd_voice("lang zh", object(), {})
        assert voice_cmd._voice_language == "zh"
        voice_cmd.cmd_voice("lang auto", object(), {})
        assert voice_cmd._voice_language == "auto"
