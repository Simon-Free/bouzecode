# [desc] Tests for ripgrep caching, file-edit safeguards, and external-modification warnings in tools.
# <tool_use name="FinalAnswer" id="fa1"><param name="answer">Tests for ripgrep caching, file-edit safeguards, and external-modification warnings in tools.</param></tool_use> [/desc]
...
def test_has_rg_is_cached():
    ...
    from bouzecode.backend.tools import _has_rg
    _has_rg.cache_clear()
    result1 = _has_rg()
    result2 = _has_rg()
...
def test_edit_allowed_without_prior_read(tmp_file):
    ...
    from bouzecode.backend.tools import _edit
...
def test_edit_allowed_after_read(tmp_file):
    ...
    from bouzecode.backend.tools import _read, _edit
    _read(tmp_file)
    result = _edit(tmp_file, "line1", "modified1")
    assert "Changes applied" in result
    assert "[Warning]" not in result
...
def test_write_updates_mtime(tmp_file):
    ...
    from bouzecode.backend.tools import _write, _edit
...
def test_clear_file_state_resets_cache(tmp_file):
    ...
    from bouzecode.backend.tools import _read, _edit, clear_file_state, _file_content_cache
    _read(tmp_file)
...
def test_dag_levels_no_deps():
    ...
    from bouzecode.backend.agent import _build_dag_levels
...
def test_propagate_denials():
    ...
    from bouzecode.backend.agent import _propagate_denials
...
def test_sanitize_messages_adds_missing_tool_results():
    ...
    from bouzecode.backend.agent.providers import sanitize_messages
