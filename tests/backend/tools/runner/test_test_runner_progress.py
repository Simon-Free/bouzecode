# [desc] Tests for RunPythonTest progress bar regex parsing and end-to-end test execution reporting [/desc]
...
class TestRunPythonTestProgress:
    ...
    def test_runs_simple_tests_and_reports(self, tmp_path):
        test_file = tmp_path / "test_progress_pass.py"  # basename unique dans la suite (pytest imbriqué)
    ...
    def test_reports_failures(self, tmp_path):
        test_file = tmp_path / "test_progress_fail.py"  # basename unique dans la suite (pytest imbriqué)
        test_file.write_text(
