# [desc] End-to-end tests for symbol-aware folder descriptions, file reading, and tool schema integration [/desc]
...
def test_e2e_read_symbol_not_found_falls_back_to_the_whole_file(symbol_project):
    """Symbole absent : plus de refus — le fichier entier est servi, l'absence est dite,
    et les symboles disponibles sont nommés."""
    fp = str(symbol_project / "mylib" / "core.py")
    result = _read(fp, symbol="nonexistent")
    assert "not found" in result
    assert "nonexistent" in result
    assert "Available symbols:" in result and "greet" in result
    assert "def greet" in result and "class DatabaseClient" in result
...
def test_e2e_system_prompt_contains_symbol_rules():
    # Symbol-aware reading guidance lives in the default (code) agent profile,
    # layered on the noyau at depth 0 by dispatch — not in the shared base prompt.
    from pathlib import Path
    ...
    # L'échelle ne prescrit plus GetFolderDescription (outil non offert aux agents) :
    # elle liste les symboles avec Grep — cf. test_prompt_registry_conformity.
    assert "GetFolderDescription" not in extra
    assert "Snippet(symbol=" in extra
