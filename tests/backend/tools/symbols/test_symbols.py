# [desc] Tests for Python symbol extraction, find_symbol lookup, Read(symbol=...) filtering, and folder description integration [/desc]
...
def test_read_symbol_not_found_serves_the_module_level_code(tmp_path):
    """Un module sans symbole nommable : on dit que le symbole est absent ET on sert
    le code de niveau module, qui est justement ce que l'agent cherchait."""
    f = tmp_path / "mod.py"
    f.write_text("x = 1\n")
    from bouzecode.backend.tools.ops.file_ops import _read
    ...
    assert "Available symbols: (no symbols detected)" in result
