# [desc] Tests unified diff generation, truncation, and diff output from edit/write operations.
# <tool_use name="FinalAnswer" id="fa1"><param name="answer">Tests unified diff generation, truncation, and diff output from edit/write operations.</param></tool_use> [/desc]
import tempfile
import pytest

def test_generate_unified_diff():
    from bouzecode.backend.tools import generate_unified_diff
    old = "line1\nline2\nline3\n"
    new = "line1\nline2_modified\nline3\n"
...
def test_write_existing_returns_diff(tmp_path):
    from bouzecode.backend.tools import _write
    f = tmp_path / "test.txt"
...
def test_diff_truncation():
    from bouzecode.backend.tools import generate_unified_diff, maybe_truncate_diff
