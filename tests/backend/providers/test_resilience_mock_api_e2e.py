# [desc] mock_api e2e: providers resilience/transport (retry on 429/5xx, malformed SSE, cut connections) driven through the REAL wire pipeline. [/desc]
...
# Threaded werkzeug server + httpx streaming dead-locks intermittently on Windows
# (reader thread hangs on a socket that never EOFs), freezing the worker. Runs on
# Linux CI.
...
def test_rate_limit_then_success_recovers():
    ...
    result = bouzecode(["hi"], mock_api=[{"status": 429}, "recovered."])
...
def test_two_server_errors_then_success_recovers():
    ...
    result = bouzecode(
        ["hi"],
        mock_api=[{"status": 500}, {"status": 500}, "ok."],
    )
...
def test_malformed_sse_event_does_not_kill_the_turn():
    """A gateway/proxy can emit an SSE event with an empty `data:` payload, which
    ...
    result = bouzecode(["hi"], mock_api=[{"raw_sse": bad}, "recovered2."])
...
def test_cut_connection_keeps_partial_and_conversation_continues():
    ...
    result = bouzecode(
        ["hi"],
        mock_api=[{"chunks": chunks, "truncate_after": 2}, "completed."],
    )
...
def test_clean_stream_completes_in_one_request():
    ...
    result = bouzecode(["hi"], mock_api=["all good."])
