# [desc] Proof tests for the mock-API harness mode: the REAL wire/SSE/parse/retry pipeline runs against a fake Anthropic server. [/desc]
...
def test_mock_api_runs_real_pipeline_and_records_wire_payload():
    result = bouzecode(["hi there"], mock_api=["Hello from the wire."])
...
def test_thinking_streamed_via_sse_lands_in_transcript():
    ...
    result = bouzecode(
        ["reason about it"],
        mock_api=[{"thinking": ["private reasoning here"], "text": "the answer."}],
    )
