# [desc] Subprocess test verifying model access denied produces clean error, not UnboundLocalError [/desc]
...
BOUZECODE_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
...
def test_model_access_denied_no_crash():
    ...
    """
    require_api_key()  # live API test — skips when unreachable
    env = os.environ.copy()
