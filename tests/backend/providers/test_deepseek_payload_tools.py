# [desc] Diagnostic test verifying DeepSeek native mode receives Methodology/Snippet tools with correct schemas. [/desc]
...
class TestDeepSeekReceivesMethodologyAndSnippet:
    ...
    def test_tool_docs_section_is_absent_in_native_mode(self, deepseek_config):
        """In native mode the XML tool docs must not be in the system prompt at all —
        the schemas travel in the API `tools` param instead."""
        payload = _get_system_payload("deepseek-v4-flash", deepseek_config, TOOL_SCHEMAS)
        system_text = "\n\n".join(
            b["text"] for b in payload.system_blocks if b.get("text")
        )
        # "## Methodology" is a tool section heading emitted only by build_tool_docs
        # (the inverse of test_compare_xml_mode_has_full_tool_docs below). Asserted on
        # content rather than on block index: empty blocks are dropped from the wire,
        # so positions shift.
        assert "## Methodology" not in system_text
        assert all(b.get("text", "").strip() for b in payload.system_blocks), (
            "empty system blocks must never reach the wire"
        )
