# [desc] Tests stream resilience helpers against malformed SSE events with JSON/Unicode decode errors
# <tool_use name="FinalAnswer" id="f1"><param name="answer">Tests stream resilience helpers against malformed SSE events with JSON/Unicode decode errors</param></tool_use> [/desc]
"""Tests for stream resilience against malformed SSE events.

Scenario: the socle/proxy in front of Anthropic can send SSE events with
