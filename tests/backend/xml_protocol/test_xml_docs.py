...
def test_docs_include_a_parsable_xml_example():
    ...
    completed = [item for item in p.feed(docs) if isinstance(item, dict)]
...
def test_docs_show_the_scheduling_attributes_in_a_form_the_parser_accepts():
    """The model imitates what it is SHOWN. Naming depends_on without showing its
    syntax is what made it invent depends_on="[...]" (50 % of all XML failures)."""
    from bouzecode.backend.xml_tool_protocol import XmlToolStreamParser
    docs = _build([])
    assert 'depends_on=["w1"]' in docs
    assert 'tool_call_alias="w1"' in docs

    p = XmlToolStreamParser()
    calls = [item for item in p.feed(docs) if isinstance(item, dict)] + p.finalize()
    chained = [c for c in calls if "depends_on" in c["input"]]
    assert chained, "the worked example must show a real chained call"
    assert chained[0]["input"]["depends_on"] == '["w1"]'


def test_the_documentation_itself_never_looks_like_a_broken_tool_call():
    """Whatever we write in the prompt, the model reads it back through this parser:
    a doc that parses to an error teaches the error."""
    from bouzecode.backend.xml_tool_protocol import XmlToolStreamParser
    docs = _build([READ_SCHEMA, WRITE_SCHEMA, BASH_SCHEMA])
    p = XmlToolStreamParser()
    calls = [item for item in p.feed(docs) if isinstance(item, dict)] + p.finalize()
    assert [c for c in calls if c["name"] == "_XmlParseError"] == []
