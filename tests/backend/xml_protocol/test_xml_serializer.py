...
def _serialize_call(tc):
    from bouzecode.backend.xml_tool_protocol import serialize_tool_call
    return serialize_tool_call(tc)


def _serialize_result(tool_call_id, content):
    from bouzecode.backend.xml_tool_protocol import serialize_tool_result
...
def _parse(xml):
    ...
    # feed() returns an interleaved list of visible text (str) and completed tools (dict).
    return [item for item in p.feed(xml) if isinstance(item, dict)]
...
def test_serialize_result_includes_token_count():
    xml_empty = _serialize_result("r1", "")
    assert 'tokens="0"' in xml_empty
    # estimate_tokens uses len/3.5 — 3500 chars ≈ 1000 tokens
    xml_big = _serialize_result("r2", "x" * 3500)
    assert 'tokens="1000"' in xml_big


def test_serialize_result_token_attr_precedes_body():
    xml = _serialize_result("b87", "some log content")
    id_pos = xml.index('id="b87"')
    tokens_pos = xml.index('tokens="')
    body_pos = xml.index("some log")
    assert id_pos < tokens_pos < body_pos
