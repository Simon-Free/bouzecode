# [desc] Feature tests: a silent Methodology-only turn continues the session instead of ending it. [/desc]
...
def test_silent_methodology_only_turn_continues():
    ...
    mock = MockLLM([
        ...
        "fini.",                    # text-only reply (no tool) closes the session
    ])
...
def test_methodology_only_with_final_text_does_not_close():
    """Final text + Methodology in the SAME batch must NOT close by default: only
    FinalAnswer closes. The batch is meta-only → the model is nudged once to
    continue, and closes on the following text-only reply (no tool calls)."""
    mock = MockLLM([f"Voilà la réponse finale.\n{METH}", "vraiment fini."])
    result = bouzecode(["question simple"], mock_llm=mock, config_overrides=NO_TEST_ENFORCE)

    assert len(_continue_nudges(result)) == 1
    # Closes on the text-only follow-up, not on the meta-only batch.
