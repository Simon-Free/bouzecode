# [desc] Conversation tests: FinalAnswer is an explicit close signal; a validator refusal keeps the session running. [/desc]
...
def test_refused_close_keeps_session_running(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda answer, config, recap=None: next(verdicts),  # validate_close prend recap (T7)
    )
