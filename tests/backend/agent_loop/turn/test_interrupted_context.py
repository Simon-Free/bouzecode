# [desc] E2E tests verifying Ctrl+C interrupted context preservation and cleanup across agent turns. [/desc]
...
def test_cleanup_removes_old_interrupted_on_new_run(monkeypatch):
    ...
    # Turn 1: a Methodology-only turn satisfies enforcement (meta-only does NOT
    # close the session since b83ade94 — it triggers a nudge). Turn 2: a plain
    # text answer closes cleanly. The legacy `done.\n{METH}` one-shot idiom is
    # dead (meta-only never closes → queue exhausted → MockLLM stream() overrun).
    mock = MockLLM([
