...
def test_meta_only_nudge_increments_counter():
    ...
    mock = MockLLM([
        ...
        METH,
        # Turn 2 (after nudge): real work resets the meta-only counter
        f'Done.\n{METH}\n<tool_use name="Bash" id="b1"><param name="command">echo ok</param></tool_use>',
        # Turn 3: text-only reply closes the session (only FinalAnswer/text-pure closes)
        "All done.",
    ])
...
def test_no_meta_only_nudge_stays_zero():
    ...
    mock = MockLLM([
        ...
        # Turn 2: text-only reply closes the session
        "Finished.",
    ])
