# [desc] Conversation feature tests for DAG depends_on ordering: a Write->Bash batch runs in dependency order (Bash sees the written file) via explicit depends_on, auto-injection, and basename matching — observed through real bouzecode() tool results. [/desc]
...
def test_explicit_depends_on_runs_write_before_bash(tmp_path):
    ...
    bash_call = _bash(f, depends_on='["w1"]')
    mock = MockLLM([
        f"{METH}\n{_write(f)}\n{bash_call}",
        CLOSE,
    ])
