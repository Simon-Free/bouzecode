# [desc] Tests for checkpoint snapshot creation, file backup tracking, rollback, and serialization roundtrips. [/desc]
"""The checkpoint store itself — unit level on purpose.

What the user does with checkpoints (review a diff, /revert a request) is covered
by conversations in test_getdiff_e2e.py. This file pins the storage layer under
it: snapshot numbering, first-write-wins backups, version bookkeeping, rollback
and JSON round-trips. These are data-structure invariants that a conversation
exercises only incidentally and could never pin precisely.
"""
...
def tmp_home(tmp_path):
    ...
    with patch("bouzecode.backend.checkpoint.store._checkpoints_root", return_value=ckpt_root):
...
def reset_versions():
    ...
    from bouzecode.backend.checkpoint.store import reset_file_versions
    reset_file_versions()
    yield
    reset_file_versions()
...
class TestTypes:
    def test_file_backup_roundtrip(self):
        from bouzecode.backend.checkpoint.types import FileBackup
    ...
    def test_snapshot_roundtrip(self):
        from bouzecode.backend.checkpoint.types import Snapshot, FileBackup
...
class TestStore:
    ...
    def test_max_snapshots_sliding_window(self, tmp_home):
        from bouzecode.backend.checkpoint import store
        from bouzecode.backend.checkpoint.types import MAX_SNAPSHOTS
        state = FakeState(messages=[], turn_count=0)
...
class TestHooks:
    def test_set_session_and_tracking(self, tmp_home, tmp_path):
        from bouzecode.backend.checkpoint import hooks, store
    ...
    def test_reset_tracked(self, tmp_home, tmp_path):
        from bouzecode.backend.checkpoint import hooks
...
class TestIntegration:
    def test_write_snapshot_rewind_cycle(self, tmp_home, tmp_path):
        ...
        from bouzecode.backend.checkpoint import store, hooks
