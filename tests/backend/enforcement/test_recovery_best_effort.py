# [desc] Feature tests: a crashing recovery side-call must not kill the session. [/desc]
...
def test_methodology_recovery_crash_does_not_kill_session(monkeypatch):
    """Un side-call de recovery qui plante ne tue pas la session : le travail du tour s'exécute quand même."""
    def _boom(*_a, **_k):
    ...
    mock = MockLLM([
        ...
        f"fini.\n{METH}",  # meta-only batch → nudged, not closed
        "Termine.",        # plain reply, no tool calls → closes
    ])
...
def test_snippet_recovery_crash_does_not_kill_session(monkeypatch, tmp_path):
    """Une recovery de Snippet qui plante laisse la conversation aller jusqu'à sa fin normale."""
