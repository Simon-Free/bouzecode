# [desc] Tests the forced Methodology/Snippet recovery that augments a batch before execution. [/desc]
...
def test_enforce_methodology_is_plain_and_dedups():
    """Un meme tool call emis deux fois n'est execute qu'une fois, sans message d'enforcement renvoye au modele."""
    state = _state([
...
def test_snippetable_results_returns_reads_with_content():
    """Le rattrapage recoit la lecture du tour concerne avec son contenu complet, meme apres un tour sans lecture."""
    msgs = [
        {"role": "user", "content": "go"},
        {"role": "assistant", "content": "", "tool_calls": [
...
def test_recover_methodology_seeds_prev_and_thinking(monkeypatch):
    """La Methodology recuperee est demandee au modele avec son raisonnement du tour et sa note precedente sous les yeux."""
    captured = {}
...
def test_recover_methodology_none_without_thinking(monkeypatch):
    """Sans raisonnement a distiller, aucune Methodology n'est inventee."""
...
def test_recover_snippets_seeds_reads_and_thinking(monkeypatch):
    """Le Snippet recupere est demande avec le contenu lu et le raisonnement du tour, et le modele est oblige de repondre."""
    captured = {}
...
def test_recover_snippets_noop_when_no_results():
    """Sans lecture a couvrir, le rattrapage ne declenche aucun appel."""
    assert recover_snippets([], _ctx(), {"model": "m"}) == []


def test_ask_forced_pins_or_requires_tool_choice(tools, monkeypatch):
    """L'appel force n'expose que l'outil vise, en payload minimal : epingle pour Methodology, obligatoire pour Snippet."""
...
def test_loop_prepends_methodology_when_missing(monkeypatch):
    ...
    # turn 2 is meta-only (Methodology alone) → nudged, so turn 3 closes plainly
...
def test_loop_appends_snippet_for_uncovered_read(monkeypatch):
    ...
    # turn1 reads; turn2 forgets Snippet (meta-only → nudged); turn3 closes plainly
    mock = MockLLM([f"{meth}\n{read}", f"Fini.\n{meth}", "Termine."])
