# [desc] E2E tests verifying Skill tool interaction with enforcement hooks and loop continuation behavior [/desc]
...
# A Methodology(+Snippet) batch is meta-only: the loop nudges instead of closing,
# so every scenario ends on a plain, tool-call-free reply.
CLOSE = "Termine."
...
def test_skill_without_snippet_triggers_recovery(monkeypatch):
    ...
    mock = MockLLM([
        ...
        METH,  # bookkeeping turn: nudged, never a close — CLOSE below ends it
        CLOSE,
    ])
    result = bouzecode(
...
def test_skill_with_snippet_completes():
    ...
    mock = MockLLM([
        ...
        # Turn 3: still meta-only → nudged; Turn 4 closes with a plain reply
        METH,  # bookkeeping turn: nudged, never a close — CLOSE below ends it
        CLOSE,
    ])
    result = bouzecode(
