# [desc] Conversation feature tests for the Methodology tool: append semantics and auto-appended user/Q&A blocks. [/desc]
...
# A Methodology-only batch is "meta-only": it no longer closes the session, so
# every scenario ends on a tool-call-free reply that lets the turn finish.
CLOSE = "C'est fait."
...
def test_methodology_content_lands_in_note():
    """Ce que l'agent écrit dans Methodology se retrouve dans sa note de travail."""
    result = bouzecode(["do X"], mock_llm=MockLLM([_meth("first finding"), CLOSE]))
    assert "first finding" in _note(result)


def test_methodology_accumulates_across_turns():
    """Deux tours de Methodology s'ajoutent l'un à l'autre au lieu de s'écraser."""
    mock = MockLLM([_meth("## Goal\nfix bug"), CLOSE, _meth("## Findings\nin foo()"), CLOSE])
...
def test_methodology_empty_content_leaves_prior_intact():
    """Un appel Methodology au contenu vide n'efface pas ce qui a déjà été noté."""
    mock = MockLLM([_meth("important note"), CLOSE, _meth(""), CLOSE])
    note = _note(bouzecode(["t1", "t2"], mock_llm=mock))
    assert "important note" in note


def test_user_messages_auto_appended_as_blocks():
    """Chaque message de l'utilisateur est archivé tel quel dans la note de méthodologie."""
    mock = MockLLM([_meth("ok"), CLOSE, _meth("ok"), CLOSE])
...
def test_snippet_block_lands_in_note(tmp_path):
    """Un Snippet émis dans le même tour que Methodology fige ses lignes dans la note."""
    f = tmp_path / "f.py"
