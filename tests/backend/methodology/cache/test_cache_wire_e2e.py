# [desc] End-to-end cache coverage via mock_api: cache_control breakpoints and the stable methodology prefix actually reach the Anthropic wire. [/desc]
...
# A Methodology-only batch is meta-only: it nudges instead of closing the turn,
# so each user message needs a following plain, tool-call-free reply to close.
CLOSE = "Termine."
...
def test_cache_control_breakpoint_reaches_the_wire():
    ...
    result = bouzecode(
        ["t1", "t2"],
        mock_api=[f"a.\n{_meth('durable project context to cache')}", CLOSE,
                  f"b.\n{_meth('more')}", CLOSE],
    )
