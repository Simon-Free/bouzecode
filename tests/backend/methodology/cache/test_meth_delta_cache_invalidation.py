# [desc] Bitwise proof that meth_delta fusion into message anchor invalidates Anthropic prompt cache each turn [/desc]
...
def test_methodology_system_block_evolves_correctly():
    ...
    # Turn 2 snapshot should NOT contain the new finding
    # (it's the OLD methodology that's cached)
...
def test_full_payload_json_prefix_diverges():
    ...
    The cache works on the TOKEN PREFIX. If the JSON diverges at byte N,
    all tokens after byte N are re-processed = cache miss on everything after.
    """
    msgs = _two_loop_messages()
    p2 = _build_provider_payload(METH_TURN2, METH_TURN1, msgs)
    p3 = _build_provider_payload(METH_TURN3, METH_TURN2, msgs)
    ...
    ctx_start = max(0, diverge_at - 80)
    ctx_end = min(min_len, diverge_at + 80)

    print(f"\n=== PAYLOAD DIVERGENCE at byte {diverge_at} / {min_len} ===")
    print(f"Turn 2: ...{j2[ctx_start:ctx_end]}...")
    print(f"Turn 3: ...{j3[ctx_start:ctx_end]}...")

    # The divergence should be in the messages section (anchor),
    # not in the system section
    system_json = json.dumps(p2["system"], ensure_ascii=False)
    system_len = len(system_json)
    print(f"\nSystem JSON length: {system_len}")
    print(f"Divergence at byte: {diverge_at}")

    # Calculate wasted cache: everything before the anchor that gets re-read
    anchor_idx = _find_anchor_index(p2["messages"])
    pre_anchor_msgs = p2["messages"][:anchor_idx]
    pre_anchor_size = len(json.dumps(pre_anchor_msgs, ensure_ascii=False))
    system_size = len(json.dumps(p2["system"], ensure_ascii=False))
    wasted = system_size + pre_anchor_size

    print(f"\n=== CACHE COST PER TURN ===")
    print(f"System blocks: {system_size:,} chars")
    print(f"Messages before anchor: {pre_anchor_size:,} chars")
    print(f"TOTAL re-tokenized every turn: {wasted:,} chars (~{wasted // 4:,} tokens)")
    print("This content SHOULD be cached but isn't because the anchor changes.")
