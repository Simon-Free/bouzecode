# [desc] Conversation feature tests for tool_id snippeting: inline doc-tool results wrapped on the wire, snippeted by tool_id, enforced. [/desc]
...
def test_tool_id_snippet_lands_in_note_and_wraps_on_wire(doc_tool):
    """Un résultat d'outil snippeté par tool_id atterrit dans la note et repart marqué sur le fil."""
    mock = MockLLM([
        ...
        # Methodology+Snippet is a meta-only batch and no longer closes the turn,
        # so the model needs a last tool-call-free reply to end the session.
        "Done.",
    ])
