...
import bouzecode.backend.tools.skill.loader as _loader
from bouzecode.backend.tools.skill.loader import _parse_skill_file, _parse_list_field, find_skill, SkillDef
from bouzecode.backend.tools.skill import load_skills, substitute_arguments
...
def skill_dir(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(_loader, "_get_skill_paths", lambda: [(skills_dir, "project")])
...
def test_load_skills_empty_dir(tmp_path, monkeypatch):
    ...
    empty.mkdir()
    monkeypatch.setattr(_loader, "_get_skill_paths", lambda: [(empty, "project")])
    monkeypatch.setattr(_loader, "_BUILTIN_SKILLS", [])
    assert load_skills() == []


def test_load_skills_nonexistent_dir(tmp_path, monkeypatch):
    monkeypatch.setattr(_loader, "_get_skill_paths", lambda: [(tmp_path / "does_not_exist", "project")])
    monkeypatch.setattr(_loader, "_BUILTIN_SKILLS", [])
    assert load_skills() == []
