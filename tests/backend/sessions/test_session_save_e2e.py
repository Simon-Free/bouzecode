# [desc] Conversation feature tests for session save: a real bouzecode() run produces thinking + tool_use + notes, then the real _build_session_data preserves thinking, strips tool_use XML, keeps a thinking-only message, and records context_state notes; /clear clears the notes. [/desc]
...
def test_save_thinking_only_message_not_dot():
    ...
    mock = MockLLM([
        {"thinking": ["Planning my approach to this task."],
         "text": f"{METH}"},
        "done.",   # the silent meta-only turn above gets a continue-nudge
    ])
...
def test_save_includes_context_state_notes_from_conversation():
    ...
    result = bouzecode(["investigate X"], mock_llm=MockLLM([meth, "done."]))
...
def test_save_no_attribute_error_on_real_state():
    ...
    result = bouzecode(["hello"], mock_llm=MockLLM([METH, "done."]))
...
def test_cmd_clear_wipes_notes_built_by_conversation(tmp_path):
    ...
    result = bouzecode(["do work"], mock_llm=MockLLM([meth, "done."]))
    state = result.state
