...
def _base_config():
    return {
        # Without an explicit permission mode every tool call comes back
        # "Denied: user rejected this operation" — including the FinalAnswer whose
        # close these tests assert. A close riding on a denied tool is no longer a
        # close (the answer would rest on something that never ran), so the config
        # has to reflect what a real session does: grant the calls.
...
def test_close_reason_no_tools_text(monkeypatch, bypass_enforcement):
    """Assistant replies with only text (no tool calls) → close_reason='text_no_tools'.
    Littéral aligné sur wake.GRACEFUL_CLOSE_REASONS (une clôture sur texte est gracieuse)."""
...
def test_close_reason_no_tools_text_after_work(monkeypatch, methodology_tool):
    ...
    session immédiatement avec close_reason='text_no_tools' — plus de tour de
