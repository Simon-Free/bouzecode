# [desc] Tests that export_wire renders the per-turn LLM wire to markdown with ## TURN / ### REQUEST / ### RESPONSE headings [/desc]
    ...
    records = [
        {
            ...
            "timestamp": 1.0,
            "system_blocks": [
                {"type": "text", "text": "APP SYSTEM PROMPT", "cache_control": {"type": "ephemeral"}},
                {"type": "text", "text": "<tools>...</tools>"},
            ],
            "messages": [
                {"role": "user", "content": "occupancy rate"},
        ...
        },
        {
            "turn": 2,
            "timestamp": 2.0,
            "system_blocks": [{"type": "text", "text": "APP SYSTEM PROMPT"}],
            "messages": [
...
def test_export_all_turns(tmp_path):
    ...
    assert "APP SYSTEM PROMPT" in md          # system block rendered (request)
