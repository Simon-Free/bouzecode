# [desc] E2E and unit tests for the thinking-out-loud pipeline: parser, loop detector, and agent loop [/desc]
...
# ---------------------------------------------------------------------------
# Ensure bouzecode root is importable
# ---------------------------------------------------------------------------
...
def _patch_agent_loop(monkeypatch, fake_stream):
    ...
    # MCP module deleted — nothing to stub here.
...
class TestThinkingStreamParser:
    ...
    def test_chunked_across_tag_boundary(self):
        p = ThinkingStreamParser()
        # Tags are recognized only at line start (their own line), even when the
        # stream is cut mid-tag across chunks.
        r1 = p.feed("<thinking>\ninsi")
        r2 = p.feed("de\n</thin")
        r3 = p.feed("king>\nafter")
        all_results = r1 + r2 + r3 + p.finalize()
        texts = [c for k, c in all_results if k == "text"]
        thinks = [c for k, c in all_results if k == "thinking"]
        assert "after" in "".join(texts)
        assert "inside" in "".join(thinks)

    def test_interleaved_blocks(self):
        p = ThinkingStreamParser()
        # Visible text between blocks sits on its own line (line-start tags).
        out = p.feed("<thinking>\nT1\n</thinking>\nB\n<thinking>\nT2\n</thinking>\nC")
        out += p.finalize()
...
class TestAgentLoopThinking:
    ...
    def test_thinking_stored_in_messages(self, monkeypatch):
        ...
        from bouzecode.backend.agent.loop import run
        state = AgentState()
        # recover_memory=False: a text-only reply (no tool call) must close the turn,
        # not trigger the no-tool recovery side-call that re-loops run() several times.
        config = {"model": "test-model", "recover_memory": False}
    ...
    def test_multi_turn_thinking(self, monkeypatch):
        ...
        from bouzecode.backend.agent.loop import run
        state = AgentState()
        # recover_memory=False: text-only replies close each turn instead of looping
        # via the no-tool recovery side-call (which would inflate assistant count).
        config = {"model": "test-model", "_context_state": state.context_state,
