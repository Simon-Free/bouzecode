# [desc] Tests that _build_assistant_content prepends thinking tags to saved assistant message content. [/desc]
...
class TestBuildAssistantContent:
    ...
    def test_inline_thinking_not_doubled(self):
        """Regression: non-native providers pass at_text with inline <thinking>
        blocks AND thinking is also captured in thinking_parts. The result must
        contain exactly ONE <thinking> block (thinking_parts is canonical), not
        two — otherwise the block renders twice in the web UI."""
        at_text = "<thinking>\ninline reasoning\n</thinking>\n\nVisible answer"
        result = _build_assistant_content(at_text, ["canonical reasoning"])
        assert result.count("<thinking>") == 1
        assert result.count("</thinking>") == 1
        assert "canonical reasoning" in result
        assert "Visible answer" in result
        # the inline duplicate must be gone
        assert "inline reasoning" not in result
