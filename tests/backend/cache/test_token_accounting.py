# [desc] E2E tests verifying per-turn and session-level cache token accounting through the agent loop [/desc]
...
def mock_loop(monkeypatch):
    ...
    def setup(responses):
        ...
        monkeypatch.setattr(ltm, "get_tool_schemas", lambda: [])
        monkeypatch.setattr(lm, "is_web_ipc_active", lambda: False)
        def _fake_exec(level, results, durations, config):
            for tc in level:
                results[tc["id"]] = f"[{tc['name']} result]"
                durations[tc["id"]] = 0.001
        monkeypatch.setattr(lm, "_execute_level", _fake_exec)
        monkeypatch.setattr(lm, "_check_permission", lambda tc, c: True)
