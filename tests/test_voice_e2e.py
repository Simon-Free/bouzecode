...
class TestVoiceDispatcherIntegration:
    ...
    def test_voice_registered_in_oss_commands(self):
        """cmd_voice is the handler registered under the 'voice' key.

        The table now stores it behind the `_echoing` adapter (it prints a shim's
        text result, which the dispatcher discards); `functools.wraps` keeps
        `__wrapped__` pointing at the shim, so `inspect.unwrap` still proves the
        identity this test is about."""
        import inspect
        from bouzecode.backend.commands.oss_shims import OSS_COMMANDS
        assert "voice" in OSS_COMMANDS
        assert inspect.unwrap(OSS_COMMANDS["voice"]) is _get_cmd_voice()
